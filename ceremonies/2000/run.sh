...
function setup_ceremony_tool() {
    ...
    if [ -n "${CEREMONY_BIN_2000}" ] && [ -x "${CEREMONY_BIN_2000}" ]; then
        export CEREMONY_BIN="${CEREMONY_BIN_2000}"
        return 0
    fi

    TOOLS="/tmp/ceremony-tools"

    # The 2000 ceremony wasn't run by Let's Encrypt, let alone using the
    # ceremony tool, so this commit is just a version that works with the
